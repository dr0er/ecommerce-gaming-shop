...
const CartItem = ({ id, name, category, price, image, amount }: ProductProps) => {
  ...
  return (
    <div className="w-auto flex h-24 justify-between ">
      <img className="bg-grey-very-light rounded-xl w-24 h-auto p-3" src={image} alt="" />
      ...
      <div className="flex flex-col justify-between">
        ...
        <div className="flex items-center gap-2">
          <img src={removeCircle} onClick={handleRemoveProduct} className="w-6" alt="" />
          <p className="font-extrabold"> {amount < 10 ? `0${amount}` : amount}</p>
          <FontAwesomeIcon icon={faPlusCircle} onClick={handleAddProduct} className="text-xl" />
        </div>
      </div>
    </div>
  )
